				...
				<div id="board">
					<div id="list">
						...
						<table >
							...
							<tbody>
								<c:forEach items="${requestScope.bList }" var="bvo" varStatus="status">
									<tr>
										<td>${bvo.no }</td>
										...
										<td>${bvo.regDate }</td>
										<c:choose>
											<c:when test="${sessionScope.authUser.name eq bvo.name }">
												<td><a href="/MySite/board?action=delete&no=${bvo.no}">[삭제]</a></td>
											</c:when>
										</c:choose>
									</tr>
								</c:forEach>
